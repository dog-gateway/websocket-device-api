...
public class DeviceWebSocketEndpoint implements EventHandler
{
    ...
    public void activate(BundleContext context)
    {
        ...
        this.websocketConnector.get().registerEndpoint(this,
                this.restApi.get());
    }
    ...
    public void handleEvent(Event event)
    {
        ...
        int connectedClients = this.websocketConnector.get()
                .getConnectedClients().size();

        // handle an event only if at least one client is connected...
        if (this.websocketConnector.get().isWebSocketAvailable()
                && connectedClients > 0)
        {
            ...
            if (eventContent != null)
            {
                ...
                if (eventContent instanceof HashSet)
                    notificationList
                            .addAll((HashSet<Notification>) eventContent);
                // single notification
                else
                ...
                for (Notification singleNotification : notificationList)
                {
                    ...
                    // (notificationContent)
                    for (Field notificationField : singleNotification.getClass()
                            .getDeclaredFields())
                    {
                        notificationField.setAccessible(true);
                        String notificationFieldName = notificationField
                                .getName();
                        ...
                        try
                        {
                                    ...
                                    .equals("notificationTopic"))
                            {
                                String[] fieldValueFinalParts = notificationFieldValueFinal
                                        .split("/");
                                notificationFieldValueFinal = fieldValueFinalParts[fieldValueFinalParts.length
                                        - 1];
                            }
                    ...
                    if (!notificationContent.isEmpty())
                    {
                        ...
                        if ((!notificationContent.get("deviceUri").isEmpty())
                                && (!eventName.isEmpty()))
                        {
                            // get all the possible keys in the format
                            // deviceUri-notificationTopic
                            Set<String> keys = this.prepareNotificationKeys(
                                    notificationContent.get("deviceUri"),
                                    eventName);
                            ...
                            for (String key : keys)
                            {
                                ...
                                {
                                    // get the clients set
                                    sendToClients.addAll(
                                            this.notification2clients.get(key));
                                }
                            }
                        }
                        ...
                        {
                            for (String client : this.websocketConnector.get()
                                    .getConnectedClients())
                            {
                                ...
                                if (sendToClients.contains(client))
                                {
                                    ...
                                    notificationResponse.setNotification(
                                            notificationContent);
                                    ...
                                    try
                                    {
                                        notificationToSend = this.mapper
                                                .writeValueAsString(
                                                        notificationResponse);
                                        // send the message
                                        this.websocketConnector.get()
                                                .sendMessage(notificationToSend,
                                                        client);
                                    }
                                    catch (Exception e)
                                    {
                                        this.logger.log(LogService.LOG_ERROR,
                                                "Exception in sending the notification"
                                                        + eventName,
                                                e);
                                    }
                                }
    ...
    @POST
    @Path("/notifications")
    public String notificationRegistrationWithoutControllable(
            Object notifications, String clientId)
    ...
    public String notificationUnregistrationWithoutControllable(
    ...
    public String notificationRegistrationWithControllable(
            @PathParam("controllable") String controllable,
            Object notifications, String clientId)
            throws JsonParseException, JsonMappingException
    {
        return this.notificationRegistration(clientId, controllable,
                notifications);
    }
    ...
    public String notificationUnregistrationWithControllable(
            @PathParam("controllable") String controllable,
            @PathParam("notification") String notification, String clientId)
    {
        return this.notificationUnregistration(clientId, "*", notification);
     ...
     * @return a {@link String} containing the result of the registration
     */
    private String notificationRegistration(String clientId,
            String controllable, Object notificationsAcquired)
            throws JsonParseException, JsonMappingException
    {
        ...
        try
        {
            notifications = this.mapper
                    .readTree((String) notificationsAcquired);
        }
        catch (Exception e)
        {
        ...
        try
        {
            ...
            else if (notifications instanceof ObjectNode)
            {
                ...
                JsonNode notificationsField = notificationsNode
                        .get("notifications");

                if (notificationsField != null
                        && notificationsField.size() == 1)
                {
                    JsonNode notificationElements = notificationsField
                            .elements().next();
                    if (notificationElements.isArray())
                    {
                        ...
                        Iterator<JsonNode> iterator = notificationArray
                                .elements();
                        ...
                        while (iterator.hasNext())
                        {
                            ...
                            notificationsList.add(current.textValue());
                        }
                    }
            ...
            // subscription
            if (this.registerNotifications(clientId, controllable,
                    notificationsList))
        ...
        {
            this.logger.log(LogService.LOG_WARNING,
                    "Impossible to convert an object in JSON", e);
     ...
     */
    private boolean registerNotifications(String clientId, String controllable,
            Set<String> notifications)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                for (String notification : notifications)
                {
                    this.notification2clients
                            .put(notification + "-" + controllable, clients);
                }
            }
        }
        catch (Exception e)
        {
            this.logger.log(LogService.LOG_ERROR,
                    "Exception in registering a new subscription for device "
                            + controllable + "from " + clientId,
                    e);
            result = false;
        }
     ...
     */
    private String notificationUnregistration(String clientId,
            String controllable, String notificationToRemove)
    {
        // set default result
        String result = "Unregistration failed";

        // the notification is a simple string (i.e., only one notification)
        if (this.removeNotification(clientId, controllable,
        ...
        catch (Exception e)
        {
            ...
            this.logger.log(LogService.LOG_WARNING,
                    "Impossible to parse the unsubscription result", e);
            return "{\"result\":\"" + result + "\"}";
        }
    }
     ...
     * @return true if the registration succeeded, false otherwise
     */
    private boolean removeNotification(String clientId,
            String controllableToRemove, String notificationToRemove)
    {
        ...
        {
            this.logger.log(LogService.LOG_ERROR,
                    "Exception in unsubscribing a notification", e);
     ...
     */
    private Set<String> prepareNotificationKeys(String device,
            String notification)
    {
